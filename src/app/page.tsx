    ...
    <div className="min-h-screen" style={{ background: 'var(--bg-creme)' }}>
      ...
      <section className="max-w-4xl mx-auto px-6 pt-20 pb-32 text-center">
        ...
        <h1 className="font-heading text-6xl md:text-7xl font-light text-[#1A1410] leading-tight mb-8">
          Echte Verbindungen
          <br />
          <em className="text-[#3B1F0A]">beginnen innen.</em>
        </h1>
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center mb-6">
          ...
          <Link href="#wie-es-funktioniert"
            className="border border-[rgba(59,31,10,0.25)] text-[#3B1F0A] font-body text-base px-10 py-4 rounded-full hover:bg-[rgba(59,31,10,0.04)] transition-colors">
            Wie es funktioniert
          </Link>
        </div>
      ...
      <section id="wie-es-funktioniert" className="py-24 px-6" style={{ background: 'var(--bg-creme)' }}>
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              {
                icon: <Users className="w-8 h-8 text-[#3B1F0A]" />,
                title: 'Profil erstellen',
                desc: 'Teile deine Werte, deinen Bindungstyp und was du wirklich suchst. Tiefgang statt Oberfläche.',
              },
              {
                icon: <Heart className="w-8 h-8 text-[#3B1F0A]" />,
                title: 'Bewusst entdecken',
                desc: 'Entdecke Profile, die wirklich zu dir passen. Bei gegenseitigem Interesse entsteht ein Match.',
              },
              {
                icon: <Sparkles className="w-8 h-8 text-[#3B1F0A]" />,
                title: 'Begegnung starten',
                desc: 'Taucht gemeinsam in die Begegnung ein — mit Tiefenfragen, Frage des Tages und vollem Fokus.',
              },
            ].map((f) => (
              <div key={f.title} className="bg-white rounded-2xl p-6 text-center" style={{ boxShadow: '0 2px 16px rgba(59,31,10,0.08)' }}>
                <div className="w-14 h-14 rounded-full bg-[rgba(59,31,10,0.07)] flex items-center justify-center mx-auto mb-5">
                  {f.icon}
                </div>
      ...
      <section className="bg-[#EDE8E0] py-24 px-6">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-6 items-start">
            ...
            {/* KOSTENLOS */}
            <div className="bg-white rounded-2xl p-6 border border-[rgba(59,31,10,0.08)]">
              ...
              <ul className="space-y-3 mb-8">
                {[
                  ...
                  <li key={item.text} className={`flex items-start gap-2.5 text-sm font-body ${item.included ? 'text-[#1A1410]' : 'text-[#A09888]'}`}>
                    {item.included
                      ? <Check className="w-4 h-4 text-[#3B1F0A] flex-shrink-0 mt-0.5" />
              ...
              <Link href="/register" className="block w-full text-center border border-[rgba(59,31,10,0.25)] text-[#3B1F0A] font-body text-sm py-3 rounded-full hover:bg-[rgba(59,31,10,0.04)] transition-colors">
                Kostenlos starten
              </Link>
            </div>

            {/* MITGLIEDSCHAFT */}
            <div className="bg-white rounded-2xl p-6 border-2 border-[#3B1F0A] relative" style={{ boxShadow: '0 8px 32px rgba(59,31,10,0.18)' }}>
              <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-[#3B1F0A] text-[#FDF5E8] text-[10px] font-body px-3 py-1 rounded-full">
                Empfohlen
              </div>
              <p className="text-xs text-[#3B1F0A] font-medium uppercase tracking-widest mb-2">Mitgliedschaft</p>
            ...
            {/* PREMIUM */}
            <div className="bg-white rounded-2xl p-6 border border-[rgba(59,31,10,0.08)]">
              <p className="text-xs text-[#3B1F0A] font-medium uppercase tracking-widest mb-2">Premium</p>
              ...
              <Link href="/pricing" className="block w-full text-center border border-[rgba(59,31,10,0.25)] text-[#3B1F0A] font-body text-sm py-3 rounded-full hover:bg-[rgba(59,31,10,0.04)] transition-colors">
                Premium starten
              </Link>
            </div>
          ...
          {/* Jahresabo Hinweis */}
          <div className="mt-10 bg-white rounded-2xl px-6 py-5 flex flex-col sm:flex-row items-center justify-between gap-4" style={{ boxShadow: '0 2px 12px rgba(59,31,10,0.08)' }}>
            <div>
          ...
          <p className="text-center mt-6 text-sm text-[#A09888] font-body">
            ...
            <Link href="/pricing" className="text-[#3B1F0A] hover:underline">Alle Pakete & Preise ansehen</Link>
          </p>
        </div>
      </section>
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-4 mb-12">
            {[
              ...
              <div key={g.title} className="bg-white rounded-2xl p-5 border border-[rgba(59,31,10,0.08)]">
                <span className="inline-block bg-[rgba(59,31,10,0.07)] text-[#3B1F0A] text-[10px] px-2.5 py-1 rounded-full mb-3 font-body font-medium">Saja Guide</span>
      ...
      {/* ── Footer ── */}
      <footer className="border-t border-[rgba(59,31,10,0.10)] py-8 px-6" style={{ background: 'var(--bg-creme)' }}>
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
          <SajaLogo size="sm" onDark={false} />
          <div className="flex gap-6 text-sm text-[#6B6058] font-body">
            <Link href="/datenschutz" className="hover:text-[#3B1F0A] transition-colors">Datenschutz</Link>
            <Link href="/impressum" className="hover:text-[#3B1F0A] transition-colors">Impressum</Link>
            <Link href="/agb" className="hover:text-[#3B1F0A] transition-colors">AGB</Link>
          </div>
