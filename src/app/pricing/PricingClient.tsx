...
const TIERS = [
  {
    ...
    monthlyPrice: 0,
    yearlyPrice: 0,
    color: 'border-[rgba(47,74,60,0.08)]/80',
    highlight: false,
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-5xl mx-auto px-6 pt-16 pb-12 text-center">
        <Link href="/" className="text-[#9A8E84] text-sm hover:text-[#232323] mb-8 inline-block">← Zurück</Link>
        <h1 className="font-heading text-5xl md:text-6xl font-light text-[#232323] mb-4">
          Transparente Preise.
        ...
        {/* Billing toggle */}
        <div className="inline-flex items-center bg-[#D8D0C7] rounded-2xl p-1 gap-1">
          <button
            onClick={() => setBilling('monthly')}
            className={cn(
              'px-5 py-2.5 rounded-xl text-sm font-medium transition-all',
              billing === 'monthly' ? 'bg-white shadow text-[#232323]' : 'text-[#6B6058] hover:text-[#232323]'
            )}
          >
            Monatlich
          </button>
          <button
            ...
            className={cn(
              ...
              billing === 'yearly' ? 'bg-white shadow text-[#232323]' : 'text-[#6B6058] hover:text-[#232323]'
            )}
          >
            Jährlich
      ...
      <div className="max-w-5xl mx-auto px-6 pb-16">
          ...
          {TIERS.map((tier) => {
            ...
            return (
              ...
              >
                ...
                <div className="mb-6">
                  <p className="text-xs text-[#9A8E84] uppercase tracking-widest mb-1">{tier.label}</p>
                  <h2 className="font-heading text-3xl text-[#232323]">{tier.name}</h2>

                  {tier.monthlyPrice === 0 ? (
                    <p className="mt-3 font-heading text-4xl text-[#232323]">0 €</p>
                  ) : (
                    <div className="mt-3">
                      <span className="font-heading text-4xl text-[#232323]">
                        {billing === 'yearly'
                          ? `${Math.round(price / 12)} €`
                          : `${price} €`}
                      </span>
                      <span className="text-[#9A8E84] text-sm ml-1">/ Monat</span>
                  ...
                  {tier.features.map((f) => (
                    <li
                      ...
                      className={cn(
                        ...
                        f.included ? 'text-[#232323]' : 'text-[#232323]/25'
                      )}
                    >
                      {f.included ? (
                ...
                <button
                  ...
                  className={cn(
                    ...
                    tier.highlight
                      ? 'bg-primary text-white hover:bg-[#2F4A3C]'
                      : tier.monthlyPrice === 0
                      ? 'border-2 border-[rgba(47,74,60,0.08)] text-[#6B6058] hover:border-primary hover:text-[#2F4A3C]'
                      : 'border-2 border-primary text-[#2F4A3C] hover:bg-primary hover:text-white'
                  )}
                >
                  {tier.cta}
                </button>
        ...
        <div className="flex items-center justify-center gap-3 mt-10 text-[#6B6058]">
          ...
          <p className="text-sm">
            <strong className="text-[#232323]">14 Tage Geld-zurück-Garantie</strong> auf alle Abonnements. Keine Fragen gestellt.
          </p>
        </div>
      </div>

      {/* One-time purchases */}
      <div className="bg-[#D8D0C7] py-20 px-6">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-12">
            <p className="text-xs text-[#9A8E84] uppercase tracking-widest mb-2">Einmalig kaufbar</p>
            <h2 className="font-heading text-4xl font-light text-[#232323] mb-3">Für immer verfügbar</h2>
          ...
          <div className="grid md:grid-cols-3 gap-4 mb-14">
            {SAJA_GUIDES.map((guide) => (
              <div key={guide.id} className="card border border-[rgba(47,74,60,0.08)]/80">
                <div className="inline-block bg-[#F2EBE2] text-[#2F4A3C] text-xs px-2.5 py-1 rounded-full mb-3 font-medium">
                  Saja Guide
                </div>
                <h4 className="font-heading text-xl text-[#232323] mb-2">{guide.title}</h4>
                <p className="text-[#232323]/55 text-sm leading-relaxed mb-5">{guide.desc}</p>
                <div className="flex items-center justify-between">
                  <span className="font-heading text-2xl text-[#232323]">{guide.price} €</span>
                  <button
      ...
      <div className="max-w-2xl mx-auto px-6 py-20">
        <h2 className="font-heading text-4xl font-light text-[#232323] text-center mb-10">Häufige Fragen</h2>
        <div className="space-y-6">
          {[
            ...
            <div key={item.q} className="border-b border-[rgba(47,74,60,0.08)] pb-6">
              <p className="font-medium text-[#232323] mb-2">{item.q}</p>
